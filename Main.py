...
def page_second():
    st.info("Для демонстрации используется датасет собранный нами посредством экспериментов с использованием iQuat.", icon="📊")
    ...
    st.write("Поехали анализировать данные! 🚀")


def page_third():
    # Гарантируем, что дефолтный датасет всегда подгружен
    if "original_data" not in st.session_state:
        df = pd.read_csv("total_data.csv", index_col=0)
        st.session_state["original_data"] = df
        st.session_state["uploaded"] = True

    # Обновляем рабочую копию данных при первом входе на страницу
...
Преобразование категориальных данных в числовые необходимо, поскольку многие алгоритмы машинного обучения могут эффективно работать только с числовыми данными. Эти методы преобразования позволяют интегрировать важную информацию о категориальных признаках в модели машинного обучения, улучшая их способность к обучению и предсказанию. Выбор метода зависит от конкретного признака и контекста задачи, и правильное применение этих техник может значительно повысить качество моделирования."""
    ...
    # Явно включаем toggle по умолчанию
    if "label_encoding_toggle" not in st.session_state:
        st.session_state["label_encoding_toggle"] = True

    label_encoding = create_toggle("label_encoding_toggle", "Label encoding", status=True)

    if label_encoding:
        label_encoding_choose = st.radio(
            "Выберите параметры для использования label encoding:",
            ["Не использовать", "Выбрать вручную", "Все параметры"],
            index=2,  # по умолчанию "Все параметры"
            key=persist("radio_for_label_encoding"),
        )

        if label_encoding_choose == "Все параметры":
            change_df = apply_label_encoder(change_df, updated_categorical_data.columns)
            ...
            updated_categorical_data = updated_categorical_data.drop(options, axis=1)
...
def page_forth():
        ...
        # Основной интерфейс — всегда отображается
...
def page_fifth():
    ...
    # Если ключа ещё нет — установим дефолтное значение
    if persist("select_y_column") not in st.session_state:
        if default_y in complete_df.columns:
            st.session_state[persist("select_y_column")] = default_y
        else:
            st.session_state[persist("select_y_column")] = complete_df.columns[0]
    ...
    # Устанавливаем значение по умолчанию, если оно ещё не задано
    if persist("type_of_task_radio") not in st.session_state:
        st.session_state[persist("type_of_task_radio")] = "Классификация"

    # Радио с выбранным значением из session_state
    ...
    if persist("checkbox_model_see") not in st.session_state:
        st.session_state[persist("checkbox_model_see")] = True
